...
| US | DOT BTS **T-100 segment, all carriers** — TranStats PREZIP bulk zips (`scripts/fetch-bts.mjs`) | Live probing (Actions runs 48–51) established that DOT's Socrata catalogs carry only *annual* T-100 summaries; the monthly airport-level table ships as PREZIP zips whose filenames carry a rotating numeric prefix. The fetcher reads the PREZIP directory listing, downloads the `T_T100_SEGMENT_ALL_CARRIER` files newest-first (bounded budget, back to 2015), unzips them with a dependency-free reader and aggregates passengers / departures / freight (lbs→tonnes) by origin × month for the ~35 largest US gateways. Socrata is still tried first (it wins automatically if DOT ever publishes a monthly table there). Best-effort with last-good fallback; total failure exits non-zero into the pipeline-health issue. Note T-100 publishes with a ~2–3 month lag, so US `latest` months trail the European feed. |
